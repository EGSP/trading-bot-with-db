import database_I = require('./databaseOperator');
...
export async function getTradeInfo(page) {
    let selectors = [];
    let text = [];
    let element;
    for(let i = 1; i < 10; i++) { if(![4,7].includes(i))selectors.push(`body > div.js-rootresizer__contents.layout-with-border-radius > div.layout__area--center > div > div.chart-container-border > div > table > tr:nth-child(1) > td.chart-markup-table.pane > div > div.legend-l31H9iuA.noWrap-l31H9iuA.noActions-l31H9iuA.wrappable-l31H9iuA > div.sourcesWrapper-l31H9iuA > div.sources-l31H9iuA > div:nth-child(3) > div.valuesWrapper-l31H9iuA > div > div:nth-child(${i}) > div`) }
...
export async function screenshotPage(symbol : string) {
